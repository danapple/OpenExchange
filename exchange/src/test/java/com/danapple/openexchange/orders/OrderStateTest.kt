...
class OrderStateTest {
    ...
    fun remainingQuantityIsReducedByTradeLeg() {
        val tradeLeg = TRADE_LEG_FACTORY.createTradeLeg(ORDER_BUY_1, 1)
    ...
    fun rejectsTradeLegWithExcessiveQuantity() {
        val tradeLeg = TRADE_LEG_FACTORY.createTradeLeg(ORDER_BUY_1, ORDER_QUANTITY_1 + 2)
    ...
    fun fillChangesOrderStatusToFilled() {
        val tradeLeg = TRADE_LEG_FACTORY.createTradeLeg(ORDER_BUY_1, ORDER_BUY_1.quantity)
        orderState1.addTradeLeg(tradeLeg)
