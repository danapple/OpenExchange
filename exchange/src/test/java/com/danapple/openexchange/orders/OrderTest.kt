...
class OrderTest {
    @Test
    fun generatesOrder() {
        val order = ORDER_FACTORY.createOrder(CUSTOMER, ORDER_TIMESTAMP_1, CL_ORD_BUY_1, INSTRUMENT_1, BigDecimal.TEN, 5)
    ...
    fun generatesOrdersWithDistinctOrderIds() {
        val order1 = ORDER_FACTORY.createOrder(CUSTOMER, ORDER_TIMESTAMP_1, CL_ORD_BUY_1, INSTRUMENT_1, BigDecimal.ONE, 5)
        val order2 = ORDER_FACTORY.createOrder(CUSTOMER, ORDER_TIMESTAMP_1, CL_ORD_BUY_1, INSTRUMENT_1, BigDecimal.ONE, 5)
    ...
    fun refusesToGenerateOrderWithZeroQuantity() {
        assertThatThrownBy { ORDER_FACTORY.createOrder(CUSTOMER, ORDER_TIMESTAMP_1, CL_ORD_BUY_1, INSTRUMENT_1, BigDecimal.ONE, 0)}.isInstanceOf(IllegalArgumentException::class.java).hasMessageContaining("must be non-zero")
    }

    @Test
    fun refusesToGenerateOrderWithZeroPrice() {
        assertThatThrownBy { ORDER_FACTORY.createOrder(CUSTOMER, ORDER_TIMESTAMP_1, CL_ORD_BUY_1, INSTRUMENT_1, BigDecimal.ZERO, 3)}.isInstanceOf(IllegalArgumentException::class.java).hasMessageContaining("must be positive")
    }

    @Test
    fun refusesToGenerateOrderWithNegativePrice() {
        assertThatThrownBy { ORDER_FACTORY.createOrder(CUSTOMER, ORDER_TIMESTAMP_1, CL_ORD_BUY_1, INSTRUMENT_1, BigDecimal.TEN.negate(), 4)}.isInstanceOf(IllegalArgumentException::class.java).hasMessageContaining("must be positive")
    }
    ...
    fun indicatesSellOrderNotBuyOrderForPositiveQuantity () {
        val order1 = ORDER_FACTORY.createOrder(CUSTOMER, ORDER_TIMESTAMP_1, CL_ORD_BUY_1, INSTRUMENT_1, BigDecimal.ONE, -7)
