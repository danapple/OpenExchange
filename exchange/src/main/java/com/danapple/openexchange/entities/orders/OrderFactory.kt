...
@Service
class OrderFactory (private val clock : Clock, private val orderIdGenerator: OrderIdGenerator,
    ...
    fun createOrder(customer: Customer, createTime: Long, clientOrderId: String, instrument: Instrument, price: BigDecimal, quantity: Int): Order {
        ...
        return Order(orderIdGenerator.getId(), customer, createTime, clientOrderId, instrument, price, quantity)
    }
    ...
    fun createOrder(customer: Customer, clientOrderId: String, newOrder: com.danapple.openexchange.dto.Order) : Order {
        ...
        return createOrder(customer , clock.millis(), clientOrderId, instrumentDao.getInstrument(leg0.instrumentId) ?: throw RuntimeException("No instrument with instrumentId ${leg0.instrumentId}"), newOrder.price, newOrder.quantity * leg0.ratio)
    }
    ...
    fun createOrder(customer: Customer, clientOrderId: String, cancelReplace: CancelReplace) : Order {
        ...
        return createOrder(customer, clock.millis(), clientOrderId, instrumentDao.getInstrument(leg0.instrumentId) ?: throw RuntimeException("No instrument with instrumentId ${leg0.instrumentId}"), cancelReplace.order.price, cancelReplace.order.quantity * leg0.ratio)
    }
}